...
export default function SchedulePage({ data, commit, notify, goTo, loadDemo, onGenerate, generating, progress }: ScheduleProps) {
  ...
  const printGrid = (kind: 'class' | 'teacher', id: string) => {
    ...
    return (
      <section className="timetable-panel bulk-print-page" key={`${kind}-${id}`}>
        <OfficialPrintHeader data={data} subtitle={`جدول ${kind === 'class' ? 'الصف' : 'المعلم'}: ${item.name}`} />
  ...
  const confirmBulkPrint = () => {
    ...
    if (!jobs.length) return notify('اختر صفاً أو معلماً أو مشرفاً واحداً على الأقل.', 'error');
    setBulkOpen(false);
    setBulkJobs(jobs);
  };
  ...
  return <div className="page-stack schedule-page">
    ...
    {!schedule ? <Panel title="الجدول الأسبوعي" description="من التفاصيل الصغيرة إلى أسبوع دراسي أكثر وضوحاً." icon={Table2}>
      <EmptyState icon={CalendarDays} title={ready ? 'كل شيء جاهز. لنصنع جدولك.' : 'جدولك المنظّم، على بُعد خطوات'} description={ready ? 'سنوزّع الحصص حسب الأنصبة، مع تجنّب تعارض المعلمين ومراعاة القيود التي أضفتها.' : 'أكمل البيانات الأساسية لتوليد جدول مدرسي، أو ابدأ جدولاً فارغاً وأدخل الحصص يدوياً بنفسك.'}>
        <Button icon={Sparkles} onClick={onGenerate} disabled={!ready} loading={generating}>إنشاء الجدول تلقائياً</Button>
        <Button variant="secondary" icon={Edit3} onClick={startManualSchedule} disabled={!data.classes.length || !data.teachers.length}>بدء جدول يدوي فارغ</Button>
        <Button variant="ghost" onClick={loadDemo} disabled={generating}>تجربة مثال جاهز<ArrowLeft size={15} /></Button>
      </EmptyState>
      ...
      <div className="schedule-controls"><div className="segmented-control"><button className={mode === 'class' ? 'selected' : ''} onClick={() => { setMode('class'); setSelectedId(data.classes[0]?.id ?? ''); }}><GraduationCap size={16} />حسب الصف</button><button className={mode === 'teacher' ? 'selected' : ''} onClick={() => { setMode('teacher'); setSelectedId(data.teachers[0]?.id ?? ''); }}><Users size={16} />حسب المعلم</button></div><select aria-label={mode === 'class' ? 'اختر الصف لعرض جدوله' : 'اختر المعلم لعرض جدوله'} value={selected?.id ?? ''} onChange={event => setSelectedId(event.target.value)} className="schedule-select">{options.map(item => <option key={item.id} value={item.id}>{item.name}</option>)}</select><div className="schedule-export"><Button variant="secondary" icon={MousePointerClick} onClick={() => goTo('distribute')}>توزيع يدوي</Button><Button variant="secondary" icon={Download} onClick={exportCsv}>تصدير CSV</Button><Button variant="secondary" icon={Users} onClick={() => setBulkOpen(true)}>طباعة متعددة</Button><Button variant="secondary" icon={Printer} onClick={() => window.print()}>طباعة</Button></div></div>
      <section className="timetable-panel">
        <OfficialPrintHeader data={data} subtitle={`جدول ${mode === 'class' ? 'الصف' : 'المعلم'}: ${selected?.name ?? ''}`} />
        ...
        <div className="table-scroll"><table className="timetable"><thead><tr><th>الحصة</th>{days.map(day => <th key={day.id}>{day.name}<span>{day.periods} حصص</span></th>)}</tr></thead><tbody>{Array.from({ length: maxPeriods }, (_, index) => index + 1).map(period => <tr key={period}><th><span className="period-number">{String(period).padStart(2, '0')}</span><span className="period-label">الحصة {period}</span></th>{days.map(day => {
          ...
          const canEditEmpty = !unavailable && period <= day.periods;
      ...
      {manualEditing && <form onSubmit={submitManual} className="modal-body">
        <div className="form-grid two-columns">
          {mode === 'class' ? (
            ...
            <Field label="الصف الدراسي"><select required value={manualEditing.classId} onChange={event => setManualEditing({ ...manualEditing, classId: event.target.value })}><option value="" disabled>اختر الصف</option>{data.classes.map(schoolClass => <option key={schoolClass.id} value={schoolClass.id}>{schoolClass.name}</option>)}</select></Field>
          )}
    ...
    <Modal open={bulkOpen} onClose={() => setBulkOpen(false)} title="طباعة عدة جداول معاً" description="اختر أي مجموعة من الصفوف أو المعلمين أو مشرفي الصفوف، وستُطبع جداولهم كاملة صفحة تلو الأخرى." wide>
      <div className="modal-body">
        <div className="bulk-print-grid">
          <div>
            <span className="field-label">الصفوف الدراسية</span>
            <CheckboxList items={data.classes.map(item => ({ id: item.id, label: item.name }))} selected={bulkSelection.classIds} onChange={ids => setBulkSelection({ ...bulkSelection, classIds: ids })} emptyLabel="لا توجد صفوف مضافة." />
          </div>
          <div>
          ...
          {data.supervisors.length > 0 && (
            <div>
              <span className="field-label">مشرفو الصفوف (تُطبع كل صفوف قسم المشرف)</span>
