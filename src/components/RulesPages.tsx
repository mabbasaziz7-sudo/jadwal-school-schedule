...
export function ExceptionsPage({ data, commit, notify, goTo, confirm }: WorkspaceProps) {
  ...
  const submit = (event: FormEvent) => {
    ...
    if (kind === 'teacher') {
      ...
      if (data.classExceptions.some(item => item.id !== form.id && item.classId === form.entityId && item.dayId === form.dayId)) return notify('يوجد انصراف مبكر لهذا الصف في اليوم نفسه. عدّله من القائمة.', 'error');
...
export function LimitsPage({ data, commit, notify, goTo, confirm }: WorkspaceProps) {
  ...
  return <div className="page-stack">
    <InlineNotice>مثال: إسناد الحصة السابعة لمعلم مرتين بالضبط أسبوعياً، بغض النظر عن اليوم أو الصف. هذه الخطوة اختيارية.</InlineNotice>
...
export function BookingsPage({ data, commit, notify, goTo, confirm }: WorkspaceProps) {
  ...
  const submit = (event: FormEvent) => {
    event.preventDefault();
    if (!ready || !form.teacherId || !form.classId || !form.dayId) return notify('اختر المعلم والصف واليوم أولاً.', 'error');
    const selectedDay = days.find(day => day.id === form.dayId);
    if (!selectedDay || form.period < 1 || form.period > selectedDay.periods) return notify('اختر يوماً مفعّلاً وحصة ضمن عدد حصص هذا اليوم.', 'error');
    ...
    if (data.classExceptions.some(item => item.classId === form.classId && item.dayId === form.dayId && form.period >= item.fromPeriod)) return notify('هذه الحصة تقع بعد وقت انصراف الصف.', 'error');
    const existing = data.bookings.filter(item => item.id !== form.id);
    if (existing.some(item => item.dayId === form.dayId && item.period === form.period && (item.teacherId === form.teacherId || item.classId === form.classId))) return notify('يوجد حجز آخر للمعلم أو الصف في الموعد نفسه.', 'error');
  ...
  return <div className="page-stack">
    {!ready && <InlineNotice kind="warning">أضف المعلمين والصفوف وفعّل أيام الدوام قبل حجز الحصص. <button className="text-link" onClick={() => goTo(!days.length ? 'days' : !data.teachers.length ? 'teachers' : 'classes')}>إكمال الإعدادات</button></InlineNotice>}
