...
export default function RequirementsPage({ data, commit, notify, goTo, confirm }: WorkspaceProps) {
  ...
  const submit = (event: FormEvent) => {
    ...
    if (!form.teacherId || !form.classId) return notify('اختر المعلم والصف أولاً.', 'error');
  ...
  return <div className="page-stack">
    ...
    <Panel title={form.id ? 'تعديل نصاب الحصص' : 'نصاب المعلم في الصف'} description="وزّع حصص كل معلم على الصفوف طوال الأسبوع، وليس لليوم الواحد." icon={ClipboardList} id="requirement-form">
      <form className="panel-form" onSubmit={submit}><fieldset disabled={!ready}>
        <div className="form-grid two-columns"><Field label="المعلم"><select required value={form.teacherId} onChange={event => setForm({ ...form, teacherId: event.target.value })}><option value="" disabled>اختر المعلم</option>{data.teachers.map(teacher => <option key={teacher.id} value={teacher.id}>{teacher.name}</option>)}</select></Field><Field label="الصف الدراسي"><select required value={form.classId} onChange={event => setForm({ ...form, classId: event.target.value })}><option value="" disabled>اختر الصف</option>{data.classes.map(schoolClass => <option key={schoolClass.id} value={schoolClass.id}>{schoolClass.name}</option>)}</select></Field></div>
        ...
        <div className="form-actions"><Button type="submit" disabled={!ready} icon={form.id ? Pencil : Plus}>{form.id ? 'حفظ التعديلات' : 'إضافة النصاب'}</Button>{form.id && <Button variant="ghost" onClick={() => setForm(blank())}>إلغاء التعديل</Button>}<p className="form-help">يمكن للمعلم تدريس أكثر من مادة للصف نفسه.</p></div>
      </fieldset></form>
    </Panel>
    {data.classes.length > 0 && <Panel title="تغطية الصفوف" description="لأسبوع متكامل، يجب أن يساوي النصاب عدد حصص الصف المتاحة." icon={BookOpenCheck} className="coverage-section"><div className="coverage-list">{data.classes.map(schoolClass => {
