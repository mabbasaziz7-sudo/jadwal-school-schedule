  ...
  return <div className="page-stack">
    ...
    <Panel title={form.id ? 'تعديل نصاب الحصص' : 'نصاب المعلم في الفصل'} description="وزّع حصص كل معلم على الفصول طوال الأسبوع، وليس لليوم الواحد." icon={ClipboardList} id="requirement-form">
      <form className="panel-form" onSubmit={submit}><fieldset disabled={!ready}>
        ...
        <div className="form-grid two-columns"><Field label="المادة الدراسية" optional hint={!data.subjects.length ? undefined : 'اختر من موادك المسجّلة، أو اكتب مادة جديدة مباشرة.'}><input maxLength={60} list="subject-suggestions" value={form.subject} placeholder="مثال: الرياضيات" onChange={event => setForm({ ...form, subject: event.target.value })} /><datalist id="subject-suggestions">{data.subjects.map(subject => <option key={subject.id} value={subject.name} />)}</datalist></Field><Field label="عدد الحصص أسبوعياً"><input required type="number" min={1} max={84} value={form.count} onChange={event => setForm({ ...form, count: Number(event.target.value) })} /></Field></div>
